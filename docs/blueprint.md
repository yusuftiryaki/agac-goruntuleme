...
- Ağaç Paneli: 'trees' Firestore koleksiyonundan getirilen, her bir liste öğesi Ağaç ID/Adı, Kayıt Tarihi ve İşlem Durumu ('pending', 'processing', 'completed') içeren taranmış ağaçların bir listesini gösteren Ana Ekran. Arayüz tamamen Türkçedir.
...
- 3D Model Görüntüleyici: Seçilen bir ağacın detay görünümü. Eğer Firestore belge durumu 'completed' ise ve 'model_url' mevcutsa, standart Google <model-viewer> web bileşenini kullanarak depodan getirilen 3D .glb modelini görüntüleyin. Durum 'Bekliyor' ise, 'Model bulutta oluşturuluyor...' yükleme mesajı gösterin.
